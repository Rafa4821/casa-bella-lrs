...
export const updateReservationStatus = async (id, status) => {
  ...
  // Auto-block dates if moving to confirmed status
  if (status === 'confirmed' && oldStatus === 'pending') {
    const { blockDatesForReservation } = await import('./dateBlockingService');
    try {
      await blockDatesForReservation({ ...reservation, id, status });
    } catch (error) {
      console.error('Error auto-blocking dates:', error);
      // Don't fail the status update if date blocking fails
    }
  }
  
  // Auto-unblock dates if cancelling a confirmed reservation
  if (status === 'cancelled' && oldStatus === 'confirmed') {
    const { deleteBlockedDate } = await import('./blockedDatesService');
    const { getAllBlockedDates } = await import('./blockedDatesService');
    try {
      const blockedDates = await getAllBlockedDates();
      const autoBlock = blockedDates.find(block => 
        block.reservationId === id && block.autoGenerated
      );
      if (autoBlock) {
        await deleteBlockedDate(autoBlock.id);
      }
    } catch (error) {
      console.error('Error auto-unblocking dates:', error);
    }
  }
};
